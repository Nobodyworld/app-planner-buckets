...
impl DesktopStoragePaths {
    fn new(root: PathBuf) -> Self {
        ...
        Self {
            primary: data_dir.join("planner-v2.json"),
            previous: data_dir.join("planner-v2.previous.json"),
            restore_recovery: backup_dir.join("restore-recovery.json"),
            migration_marker: root.join("migration-v1.complete"),
            data_dir,
            backup_dir,
        }
    }
    fn ensure_directories(&self) -> Result<(), String> {
        fs::create_dir_all(&self.data_dir)
            .map_err(|e| format!("Could not create planner data directory: {e}"))?;
        fs::create_dir_all(&self.backup_dir)
            .map_err(|e| format!("Could not create planner backup directory: {e}"))
    }
}
struct WriterGuard {
    _file: File,
...
impl RuntimeState {
    fn begin_session(&mut self) -> Result<u64, String> {
        self.session = self
            ...
            .ok_or("Storage session limit exceeded.")?;
    ...
    fn verify_session(&self, session: u64) -> Result<(), String> {
        if session > 0 && session == self.session {
            ...
            Err("Stale desktop storage session. Reload before saving.".into())
        }
    }
}
...
impl DesktopStorageState {
    pub fn new(root: PathBuf) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            writer_available: guard.is_some(),
            _writer_guard: guard,
            writer_warning: warning,
    ...
    fn lock_runtime(&self) -> Result<MutexGuard<'_, RuntimeState>, String> {
        self.runtime
            .lock()
            .map_err(|_| "Desktop storage lock failed.".into())
    }
    fn lock_writer(&self, session: u64) -> Result<MutexGuard<'_, RuntimeState>, String> {
        if !self.writer_available {
            return Err(self
                .writer_warning
                .clone()
                .unwrap_or_else(|| "Desktop storage is read-only.".into()));
        }
        let runtime = self.lock_runtime()?;
        runtime.verify_session(session)?;
        Ok(runtime)
    }
    pub fn close_requires_flush(&self) -> bool {
        self.close_guard.load(Ordering::SeqCst) && !self.allow_close.load(Ordering::SeqCst)
    }
}
fn nanos() -> u128 {
    SystemTime::now()
...
fn local_day_is_valid(day: &str) -> bool {
    ...
    year > 0
        && (1..=12).contains(&month)
        && date > 0
        && date <= days[month.saturating_sub(1).min(11)]
}
...
fn unique_path(destination: &Path, kind: &str) -> Result<PathBuf, String> {
    let parent = destination
        .parent()
        .ok_or("Storage destination has no parent.")?;
...
fn write_new_synced(path: &Path, bytes: &[u8]) -> Result<(), String> {
    ...
    file.write_all(bytes)
        .map_err(|e| format!("Could not write recovery data: {e}"))?;
    file.flush()
        .map_err(|e| format!("Could not flush recovery data: {e}"))?;
    file.sync_all()
        .map_err(|e| format!("Could not synchronize recovery data: {e}"))
}
...
fn replace_with_fault(
    ...
    previous: &Path,
    bytes: &[u8],
    fault: &mut dyn FnMut(ReplaceStep) -> Result<(), String>,
) -> Result<(), String> {
    ...
    if fs::read(&temp).map_err(|e| e.to_string())? != bytes {
        return Err("Temporary file did not verify; existing files were retained.".into());
    }
    ...
    let rollback = if previous.exists() {
        unique_path(destination, "rollback")?
    } else {
    ...
    if had_destination {
        ...
        fs::rename(destination, &rollback)
            .map_err(|e| format!("Could not preserve the previous file: {e}"))?;
    }
    let promotion = (|| {
        fault(ReplaceStep::Promote)?;
        fs::rename(&temp, destination)
            .map_err(|e| format!("Could not promote temporary file: {e}"))?;
        fault(ReplaceStep::Verify)?;
        if fs::read(destination).map_err(|e| e.to_string())? != bytes {
            return Err("Promoted file did not verify.".into());
        }
        Ok(())
    })();
    if let Err(error) = promotion {
        if had_destination {
            ...
            if let Err(rollback_error) = rollback_result {
                return Err(format!("{error} Rollback also failed ({rollback_error}); recovery files were preserved."));
            }
        } else {
...
fn replace_bytes(destination: &Path, previous: &Path, bytes: &[u8]) -> Result<(), String> {
    replace_with_fault(destination, previous, bytes, &mut |_| Ok(()))
}
fn write_auxiliary(path: &Path, bytes: &[u8]) -> Result<(), String> {
    replace_bytes(path, &path.with_extension("previous.json"), bytes)
}
fn write_primary(
    paths: &DesktopStoragePaths,
    serialized: &str,
    ...
    if !local_day_is_valid(day) {
        return Err("Invalid local calendar day.".into());
    }
    paths.ensure_directories()?;
    let existing = read_optional(&paths.primary)?;
    if existing.as_deref() == Some(serialized.as_bytes()) {
        return Ok(false);
    }
    if routine {
        if let Some(before) = &existing {
            ...
            if !snapshot.exists() {
                ...
                if fs::read(&snapshot).map_err(|e| e.to_string())? != *before {
                    return Err("Routine snapshot did not verify; primary was not replaced.".into());
                }
            }
        }
    }
...
fn operation_snapshot(
    ...
    validate_planner_json(serialized)?;
    paths.ensure_directories()?;
    let path = paths.backup_dir.join(format!(
        "operation-{}-{}-{}.json",
        sanitize(timestamp),
        sanitize(reason),
        nanos()
    ...
    if read_text(&path)? != serialized {
        return Err("Operation snapshot did not verify.".into());
    }
...
fn preserve_corrupt_primary(
    ...
    let Some(bytes) = read_optional(&paths.primary)? else {
        return Ok(None);
    };
    let path = paths.backup_dir.join(format!(
    ...
    if fs::read(&path).map_err(|e| e.to_string())? != bytes {
        return Err("Corrupt-source preservation did not verify.".into());
    }
    Ok(Some(path))
}
fn candidate_value(path: &Path, kind: &str) -> Result<Option<Value>, String> {
    ...
    Ok(Some(
        json!({ "kind": kind, "name": path.file_name().and_then(|s| s.to_str()).unwrap_or(""), "path": path_text(path), "serialized": serialized, "modifiedAtMs": modified_millis(path).min(u64::MAX as u128) as u64 }),
    ))
}
fn list_backup_candidates(paths: &DesktopStoragePaths) -> Result<Vec<Value>, String> {
    let mut values = Vec::new();
    if let Some(value) = candidate_value(&paths.previous, "previous")? {
        values.push(value);
    }
    for entry in fs::read_dir(&paths.data_dir).map_err(|e| e.to_string())? {
        ...
        if name.starts_with("planner-v2.rollback-") && name.ends_with(".json") {
            if let Some(value) = candidate_value(&entry.path(), "previous")? {
                values.push(value);
            }
        }
    }
    for entry in fs::read_dir(&paths.backup_dir).map_err(|e| e.to_string())? {
        let entry = entry.map_err(|e| e.to_string())?;
        let name = entry.file_name().to_string_lossy().into_owned();
        let kind = if name.starts_with("routine-") {
            Some("routine")
        } else if name.starts_with("operation-") {
            Some("operation")
        } else {
            None
        };
        if name.ends_with(".json") {
            if let Some(kind) = kind {
                if let Some(value) = candidate_value(&entry.path(), kind)? {
                    values.push(value);
                }
            }
        }
...
fn field<'a>(value: &'a Value, name: &str) -> Result<&'a str, String> {
    value
        ...
        .ok_or_else(|| format!("Missing storage request field: {name}"))
}
fn backup_name(name: &str) -> bool {
    !name.contains(['/', '\\'])
        && !name.contains("..")
        && name.ends_with(".json")
        && (name.starts_with("routine-") || name.starts_with("operation-"))
}
fn prune_verified(paths: &DesktopStoragePaths, request: &Value) -> Result<(), String> {
    let current = field(request, "current")?;
    if read_text(&paths.primary)? != current {
        return Err("Primary changed; retention was deferred.".into());
    }
    validate_planner_json(current)?;
    let proofs = request
        .get("candidates")
        .and_then(Value::as_array)
        .ok_or("Missing verified backup inventory.")?;
    let mut names = std::collections::BTreeSet::new();
    for proof in proofs {
        let name = field(proof, "name")?;
        let serialized = field(proof, "serialized")?;
        if !backup_name(name) {
            return Err("Invalid backup name; retention was stopped.".into());
        }
        ...
        if read_text(&paths.backup_dir.join(name))? != serialized {
            return Err("Backup changed; retention was deferred.".into());
        }
        validate_planner_json(serialized)?;
        names.insert(name.to_string());
    }
    for (prefix, retain) in [
        ("routine-", ROUTINE_RETENTION),
        ("operation-", OPERATION_RETENTION),
    ] {
        for name in names
            .iter()
            .rev()
            ...
            .skip(retain)
        {
            remove_if_exists(&paths.backup_dir.join(name))?;
        }
    }
    Ok(())
}
fn commit_restore(
    ...
    let sequence = request
        ...
        .ok_or("Invalid Restore sequence.")?;
    if sequence <= runtime.last_committed_sequence {
        return Err("Stale Restore sequence.".into());
    }
    ...
    let actual: Value = serde_json::from_str(&read_text(&paths.primary)?)
        .map_err(|_| "Primary is corrupt; Restore was stopped.")?;
    let expected_value: Value =
        serde_json::from_str(expected).map_err(|_| "Invalid expected planner.")?;
    if actual != expected_value {
        return Err("Planner changed since Restore began; nothing was replaced.".into());
    }
    if !backup_name(snapshot_name)
        || !snapshot_name.starts_with("operation-")
        || read_text(&paths.backup_dir.join(snapshot_name))? != expected
    {
        return Err("Pre-Restore snapshot did not match the current planner.".into());
    }
    let old_recovery = read_optional(&paths.restore_recovery)?;
    let recovery = match request.get("recovery") {
        Some(Value::Null) => None,
        Some(Value::String(value)) => Some(value),
        _ => return Err("Invalid Restore recovery record.".into()),
    };
    if let Some(record) = recovery {
        let parsed: Value =
            serde_json::from_str(record).map_err(|_| "Malformed Restore recovery record.")?;
        if parsed["format"] != "bsp-planner-restore-recovery"
            || parsed["previousData"] != expected_value
            || !parsed["replacementFingerprint"].is_string()
        {
            return Err("Restore recovery record did not match the transaction.".into());
        }
        write_auxiliary(&paths.restore_recovery, record.as_bytes())?;
    }
    if let Err(error) = write_primary(paths, replacement, day, true) {
        let reverted = match old_recovery {
            Some(bytes) => write_auxiliary(&paths.restore_recovery, &bytes),
            None => remove_if_exists(&paths.restore_recovery),
        };
        return Err(match reverted {
            Ok(()) => error,
            Err(other) => format!("{error} Recovery-record rollback also failed: {other}"),
        });
    }
    runtime.last_committed_sequence = sequence;
    let warning = if recovery.is_none() {
        remove_if_exists(&paths.restore_recovery)
            .err()
            .map(|e| format!("Planner committed; stale Undo record cleanup failed: {e}"))
    } else {
        None
    };
    Ok(
        json!({ "sequence": sequence, "saved": true, "stale": false, "noOp": false, "savedAt": saved_at, "warning": warning }),
    )
}
...
pub fn desktop_storage_save(
    ...
    if sequence <= runtime.last_committed_sequence {
        return Err("Stale save sequence.".into());
    }
...
pub fn desktop_storage_recover(
    ...
    Ok(
        json!({ "recovered": true, "preservedCorruptPath": preserved.map(|p| path_text(&p)) })
            .to_string(),
    )
}
#[tauri::command]
pub fn desktop_storage_create_operation_snapshot(
    ...
    Ok(
        json!({ "name": path.file_name().and_then(|n| n.to_str()), "path": path_text(&path) })
            .to_string(),
    )
}
#[tauri::command]
pub fn desktop_storage_commit_restore(
    request: String,
    session: u64,
    state: State<'_, DesktopStorageState>,
) -> Result<String, String> {
    let request: Value =
        serde_json::from_str(&request).map_err(|_| "Malformed Restore request.")?;
    let mut runtime = state.lock_writer(session)?;
...
pub fn desktop_storage_read_restore_recovery(
    ...
    runtime.verify_session(session)?;
    read_optional(&state.paths.restore_recovery)?
        .map(|bytes| {
            String::from_utf8(bytes).map_err(|_| "Restore recovery contains invalid UTF-8.".into())
        })
        .transpose()
}
#[tauri::command]
pub fn desktop_storage_clear_restore_recovery(
    ...
    let _runtime = state.lock_writer(session)?;
    remove_if_exists(&state.paths.restore_recovery)
}
#[tauri::command]
pub fn desktop_storage_list_backups(
    session: u64,
    state: State<'_, DesktopStorageState>,
) -> Result<String, String> {
    let runtime = state.lock_runtime()?;
...
#[tauri::command]
pub fn desktop_storage_prune_backups(
    request: String,
    session: u64,
    state: State<'_, DesktopStorageState>,
) -> Result<(), String> {
    ...
    prune_verified(
        &state.paths,
        &serde_json::from_str(&request).map_err(|_| "Malformed retention proof.")?,
    )
}
#[tauri::command]
pub fn desktop_storage_enable_close_guard(state: State<'_, DesktopStorageState>) {
    state.close_guard.store(true, Ordering::SeqCst);
}
#[tauri::command]
pub fn desktop_storage_finish_close(
    window: tauri::Window,
    state: State<'_, DesktopStorageState>,
) -> Result<(), String> {
...
mod tests {
    ...
    impl TestRoot {
        ...
        fn paths(&self) -> DesktopStoragePaths {
            let p = DesktopStoragePaths::new(self.0.clone());
    ...
    fn planner(label: &str) -> String {
        json!({ "version": 2, "projects": [{"id": label, "name": label, "description":"", "priority":0, "pinned":false, "createdAt":"2026-08-28T00:00:00Z", "updatedAt":"2026-08-28T00:00:00Z"}], "buckets":[], "tasks":[], "templates":[], "templateDefinitions":[] }).to_string()
    }
    #[test]
    fn daily_snapshot_is_first_prechange_and_identical_save_is_noop() {
        ...
        write_primary(&p, &b, "2026-08-28", true).unwrap();
        write_primary(&p, &planner("c"), "2026-08-28", true).unwrap();
        assert_eq!(
            read_text(&p.backup_dir.join("routine-2026-08-28.json")).unwrap(),
            a
        );
    }
    #[test]
    fn failed_recovery_keeps_the_only_previous_file() {
        for stage in [
            ...
            let root = TestRoot::new();
            let p = root.paths();
            let old = planner("previous");
            fs::write(&p.previous, &old).unwrap();
            assert!(replace_with_fault(
                &p.primary,
                &p.previous,
                planner("new").as_bytes(),
                &mut |s| if s == stage {
                    Err("injected".into())
                } else {
                    Ok(())
    ...
    fn failed_rollback_leaves_discoverable_recovery() {
        ...
        let old = planner("old");
        fs::write(&p.primary, &old).unwrap();
        fs::write(&p.previous, planner("older")).unwrap();
        assert!(replace_with_fault(
            &p.primary,
            &p.previous,
            planner("new").as_bytes(),
            &mut |s| if matches!(s, ReplaceStep::Promote | ReplaceStep::Rollback) {
                Err("injected".into())
            } else {
                Ok(())
            }
        ...
        .is_err());
        assert!(list_backup_candidates(&p)
            .unwrap()
            .iter()
            .any(|v| v["serialized"] == old));
    }
    #[test]
    fn successful_recovery_removes_previous_only_after_verified_primary() {
        ...
        let old = planner("old");
        fs::write(&p.previous, &old).unwrap();
        replace_bytes(&p.primary, &p.previous, old.as_bytes()).unwrap();
        assert_eq!(read_text(&p.primary).unwrap(), old);
        assert!(!p.previous.exists());
    }
    #[test]
    fn invalid_utf8_is_present_and_preserved_exactly() {
        let root = TestRoot::new();
        let p = root.paths();
        let bytes = [0xff, 0xfe, 0, 0x7b];
        fs::write(&p.primary, bytes).unwrap();
        assert!(candidate_value(&p.primary, "primary").unwrap().is_some());
        let preserved = preserve_corrupt_primary(&p, "test").unwrap().unwrap();
        assert_eq!(fs::read(preserved).unwrap(), bytes);
    }
    #[test]
    fn partial_version_only_json_is_not_complete_planner_data() {
        assert!(validate_planner_json("{\"version\":2}").is_err());
        assert!(validate_planner_json(&planner("valid")).is_ok());
    }
    #[test]
    fn invalid_calendar_dates_are_rejected() {
        assert!(local_day_is_valid("2024-02-29"));
        assert!(!local_day_is_valid("2026-02-29"));
        assert!(!local_day_is_valid("2026-07-00"));
        assert!(!local_day_is_valid("../../bad"));
    }
    #[test]
    fn reload_starts_new_session_and_rejects_old_writer() {
        let mut state = RuntimeState::default();
        let first = state.begin_session().unwrap();
        state.last_committed_sequence = 27;
        let next = state.begin_session().unwrap();
        assert!(state.verify_session(first).is_err());
        assert!(state.verify_session(next).is_ok());
        assert_eq!(state.last_committed_sequence, 0);
    }
    #[test]
    fn writer_handle_is_scoped_to_data_root_and_released_on_drop() {
        ...
        let pa = a.paths();
        let pb = b.paths();
        let first = acquire_writer_guard(&pa).unwrap().unwrap();
        assert!(acquire_writer_guard(&pa).unwrap().is_none());
        let second_root = acquire_writer_guard(&pb).unwrap().unwrap();
        drop(first);
        assert!(acquire_writer_guard(&pa).unwrap().is_some());
        drop(second_root);
    }
    #[cfg(windows)]
    #[test]
    fn stale_lockfile_is_not_a_stale_writer_on_windows() {
        let root = TestRoot::new();
        let p = root.paths();
        fs::write(p.data_dir.join("planner-storage.lock"), "old").unwrap();
        assert!(acquire_writer_guard(&p).unwrap().is_some());
    }
    #[test]
    fn retention_requires_current_primary_and_exact_verified_candidates() {
        let root = TestRoot::new();
        let p = root.paths();
        let data = planner("retention");
        write_primary(&p, &data, "2026-08-28", false).unwrap();
        let mut candidates = Vec::new();
        for i in 1..=35 {
            let name = format!(
                "routine-2026-{:02}-{:02}.json",
                1 + (i - 1) / 28,
                1 + (i - 1) % 28
            );
            fs::write(p.backup_dir.join(&name), &data).unwrap();
            candidates.push(json!({"name":name,"serialized":data}));
        }
        for i in 1..=18 {
            let name = format!("operation-2026-08-28T00-00-{i:02}-restore.json");
            fs::write(p.backup_dir.join(&name), &data).unwrap();
            candidates.push(json!({"name":name,"serialized":data}));
        }
        let unknown = p.backup_dir.join("routine-unknown.json");
        fs::write(&unknown, "corrupt").unwrap();
        assert!(prune_verified(
            &p,
            &json!({"current":planner("stale"),"candidates":candidates})
        )
        .is_err());
        prune_verified(&p, &json!({"current":data,"candidates":candidates})).unwrap();
        let names = fs::read_dir(&p.backup_dir)
            .unwrap()
        ...
        assert_eq!(
            names
                .iter()
                .filter(|n| n.starts_with("routine-") && **n != "routine-unknown.json")
                .count(),
            30
        );
        assert_eq!(
            names.iter().filter(|n| n.starts_with("operation-")).count(),
            12
        );
        assert_eq!(read_text(&unknown).unwrap(), "corrupt");
    }
    #[test]
    fn changed_backup_and_path_traversal_block_pruning() {
        ...
        assert!(prune_verified(
            &p,
            &json!({"current":data,"candidates":[{"name":"../planner-v2.json","serialized":data}]})
        )
        .is_err());
    ...
    fn restore_compares_current_planner_and_verified_snapshot_before_replacement() {
        let root = TestRoot::new();
        let p = root.paths();
        let before = planner("before");
        let after = planner("after");
        write_primary(&p, &before, "2026-08-28", false).unwrap();
        let request = restore_request(&p, &before, &after);
        fs::write(&p.primary, planner("external-change")).unwrap();
        let mut state = RuntimeState::default();
        assert!(commit_restore(&p, &mut state, &request).is_err());
        assert_eq!(read_text(&p.primary).unwrap(), planner("external-change"));
        fs::write(&p.primary, &before).unwrap();
        assert_eq!(
            commit_restore(&p, &mut state, &request).unwrap()["saved"],
            true
        );
        assert_eq!(read_text(&p.primary).unwrap(), after);
    }
    #[test]
    fn undo_recovery_is_not_removed_when_restore_replacement_fails() {
        ...
        let before = planner("before");
        write_primary(&p, &before, "2026-08-28", false).unwrap();
        fs::write(&p.restore_recovery, "old recovery").unwrap();
        let mut request = restore_request(&p, &before, &planner("after"));
        request["localDay"] = json!("invalid-day");
        assert!(commit_restore(&p, &mut RuntimeState::default(), &request).is_err());
        assert_eq!(read_text(&p.primary).unwrap(), before);
        assert_eq!(read_text(&p.restore_recovery).unwrap(), "old recovery");
    }
}
